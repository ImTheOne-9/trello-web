...
function ActiveCard() {
  ...
  // Func dung chung cho cac case update card title, desc, cover
  const callApiUpdatedCard = async (updateData) => {
  ...
  const onUpdateCardTitle = (newTitle) => {
    // Gọi API...
    callApiUpdatedCard({ title: newTitle.trim() })
  }

  const onUpdateCardDescription = (newDescription) => {
    // Gọi API...
    callApiUpdatedCard({ description: newDescription })
  }
  ...
  const onUploadCardCover = (event) => {
    ...
    // Gọi API...
    toast.promise(
      callApiUpdatedCard(reqData).finally(() => event.target.value === ''),
      { pending: 'Updating...' }
    )
  }

  const onAddCardComment = async (commentToAdd) => {
    await callApiUpdatedCard({ commentToAdd })
  }

  const onUpdateCardMember = (inComingMemberInfo) => {
    callApiUpdatedCard({ inComingMemberInfo })
  }
          ...
          <Grid size={{ xs: 12, sm: 9 }}>
            <Box sx={{ mb: 3 }}>
              ...
              <CardUserGroup
                ...
                handleUpdateCardMember={onUpdateCardMember}/>
            </Box>
